...
// Por cliente se configura: { banco: string, porcentaje: number }[]
// Si es nómina: base = monto total (sin quitar IVA)
// Si es normal: base = monto / 1.16

function calcMov(monto, tipo, cliente, banco, esNomina) {
  const m = parseFloat(monto)||0;
  if (tipo !== "ingreso" || !cliente || !banco) return { montoSinIVA:0, comision:0, montoFinal:m, pct:0, base:m };

  // Buscar % configurado para ese banco en el cliente
  const config = (cliente.bancos||[]).find(b => b.banco === banco);
  const pct = config ? config.porcentaje : 0;

  let base, montoSinIVA;
  if (esNomina) {
    // Nómina: comisión sobre monto total
    base = m;
    montoSinIVA = m; // no aplica IVA
  } else {
    // Normal: comisión sobre monto sin IVA
    montoSinIVA = m / 1.16;
    base = montoSinIVA;
  }

  const comision = base * (pct / 100);
  const montoFinal = m - comision;
  return { montoSinIVA, comision, montoFinal, pct, base };
}
...
// ══ PREVIEW CÁLCULO ══
function Calc({ monto, tipo, cliente, banco, esNomina }) {
  ...
  const configBanco = (cliente?.bancos||[]).find(b=>b.banco===banco);
  if (!configBanco) return (
    <div style={{background:"#fff3e0",borderRadius:10,padding:"10px 14px",marginBottom:12,borderLeft:`4px solid ${C.gold}`}}>
      <span style={{fontSize:12,color:"#e65100"}}>⚠️ Este cliente no tiene configurado {banco}</span>
    </div>
  );

  return (
    <div style={{background:C.xblue,borderRadius:10,padding:"12px 14px",marginBottom:12,borderLeft:`4px solid ${C.blue}`}}>
      <div style={{fontSize:11,color:C.blue,fontWeight:"bold",marginBottom:8,textTransform:"uppercase",letterSpacing:1}}>
        {esNomina ? "🧾 Nómina" : "💸 Transferencia"} — {banco} ({c.pct}%)
      </div>
      ...
      {!esNomina && <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
        <span style={{fontSize:12,color:C.red}}>
          Comisión {c.pct}% {esNomina?"(sobre total)":"(sobre subtotal)"}
        </span>
      ...
      <div style={{borderTop:`1px solid ${C.border}`,paddingTop:8,display:"flex",justifyContent:"space-between"}}>
        <span style={{fontWeight:"bold",color:C.navy}}>Monto final al cliente</span>
...
// ══ CALCULADORA RÁPIDA ══
function CalculadoraRapida({ clientes }) {
  ...
  const cal = useMemo(()=>calcMov(monto,"ingreso",cliente,banco,esNomina),[monto,cliente,banco,esNomina]);
    ...
    <div style={{...card,border:`2px solid ${C.blue}`}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:10}}>
        <div style={{flex:1}}>
          ...
          <select value={clienteId} onChange={e=>{setClienteId(e.target.value);setBanco("");}} style={{...inp}}>
        ...
        <div style={{flex:1}}>
          ...
          <select value={banco} onChange={e=>setBanco(e.target.value)} style={{...inp}} disabled={!clienteId}>
            <option value="">— Banco —</option>
            {bancosCliente.map(b=><option key={b.banco} value={b.banco}>{b.banco} ({b.porcentaje}%)</option>)}
          </select>
        </div>
      </div>
      <div style={{display:"flex",gap:8,marginBottom:10,alignItems:"flex-end"}}>
        <div style={{flex:1}}>
          ...
          <input inputMode="decimal" placeholder="0.00" value={monto} onChange={e=>setMonto(e.target.value)} style={{...inp,fontSize:16}} />
        </div>
        <button onClick={()=>setEsNomina(!esNomina)} style={{...btnS(esNomina?C.navy:C.lgray, esNomina?"#fff":C.gray),padding:"9px 14px",whiteSpace:"nowrap",marginBottom:1}}>
          {esNomina?"🧾 Nómina":"💸 Normal"}
        </button>
      </div>
      {parseFloat(monto)>0&&banco&&cliente&&<Calc monto={monto} tipo="ingreso" cliente={cliente} banco={banco} esNomina={esNomina} />}
    </div>
  );
}
...
function FormCliente({ ini, onSave }) {
  ...
  function setBancoPct(banco, pct) {
    setBancos(prev => {
      const existe = prev.find(b=>b.banco===banco);
      if (existe) return prev.map(b=>b.banco===banco?{...b,porcentaje:parseFloat(pct)||0}:b);
      return [...prev, {banco, porcentaje:parseFloat(pct)||0}];
    });
  }
  ...
  function toggleBanco(banco) {
    setBancos(prev => {
      const existe = prev.find(b=>b.banco===banco);
      if (existe) return prev.filter(b=>b.banco!==banco);
      return [...prev, {banco, porcentaje:3}];
    });
  }
  ...
  function guardar() {
    ...
    setErr("");
    onSave({id:ini?.id||uid(),nombre:nombre.trim(),bancos,saldoInicial:parseFloat(saldo0)||0,notas,activo:true,fechaCreacion:ini?.fechaCreacion||today()});
  }

  return (
    <div>
      <span style={lbl}>Nombre *</span>
      <input value={nombre} onChange={e=>setNombre(e.target.value)} placeholder="Nombre del cliente" style={{...inp,marginBottom:14}} />
      ...
      <div style={{fontSize:11,color:C.gray,marginBottom:10}}>Activa los bancos que aplican a este cliente y configura su %</div>
        ...
        return (
          <div key={banco} style={{background:activo?C.xblue:C.lgray,borderRadius:10,padding:"12px 14px",marginBottom:8,border:`1.5px solid ${activo?C.blue:C.border}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <button onClick={()=>toggleBanco(banco)} style={{background:activo?C.blue:C.border,border:"none",borderRadius:20,width:24,height:24,cursor:"pointer",color:"#fff",fontSize:12,flexShrink:0}}>
                {activo?"✓":""}
              </button>
              ...
              {activo && (
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  {["3","4"].map(p=>(
                    <button key={p} onClick={()=>setBancoPct(banco,p)} style={{padding:"4px 12px",borderRadius:8,border:`2px solid ${config?.porcentaje?.toString()===p?C.navy:C.border}`,background:config?.porcentaje?.toString()===p?C.navy:C.white,color:config?.porcentaje?.toString()===p?"#fff":C.gray,fontWeight:"bold",cursor:"pointer",fontSize:13}}>{p}%</button>
                  ))}
                  <input type="number" placeholder="%" min="0" max="100" value={!["3","4"].includes(config?.porcentaje?.toString())?config?.porcentaje||"":""} onChange={e=>setBancoPct(banco,e.target.value)} style={{...inp,width:60,padding:"4px 8px",fontSize:13}} />
                </div>
              )}
            </div>
          </div>
...
function FormMov({ clientes, cuentas, ini, onSave, onClose, clientesRecientes }) {
  ...
  const clientesOrdenados = useMemo(()=>{
    const rec = clientesRecientes||[];
    return [...clientes].sort((a,b)=>{
      const ia=rec.indexOf(a.id), ib=rec.indexOf(b.id);
  ...
  function guardar() {
    ...
    setErr("");
    onSave({
      id:ini?.id||uid(), tipo, clienteId:clienteId||null, cuentaId,
      banco:banco||null, esNomina, concepto, categoria, notas, fecha, estado,
      montoOriginal:m, ...cal,
      historial:ini?.historial||[], revisado:ini?.revisado||false
    });
  }
  ...
  return (
    <div>
      ...
      <select value={clienteId} onChange={e=>{setClienteId(e.target.value);setBanco("");}} style={{...inp,marginBottom:12}}>
        <option value="">— Sin cliente —</option>
        {clientesOrdenados.map((c,i)=><option key={c.id} value={c.id}>{i===0&&(clientesRecientes||[]).includes(c.id)?"⭐ ":""}{c.nombre}</option>)}
      </select>

      {/* Banco y tipo de operación — solo si hay cliente y es ingreso */}
      {tipo==="ingreso"&&clienteId&&(
        <>
          <span style={lbl}>Banco de origen *</span>
          <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
            {bancosCliente.length===0 ? (
              <div style={{color:C.red,fontSize:12,padding:"8px 12px",background:C.lred,borderRadius:8,width:"100%"}}>⚠️ Este cliente no tiene bancos configurados</div>
            ) : bancosCliente.map(b=>(
              <button key={b.banco} onClick={()=>setBanco(b.banco)} style={{padding:"8px 14px",borderRadius:9,border:`2px solid ${banco===b.banco?C.blue:C.border}`,background:banco===b.banco?C.lblue:C.lgray,color:banco===b.banco?C.navy:C.gray,fontWeight:"bold",fontSize:12,cursor:"pointer"}}>
                🏦 {b.banco} <span style={{color:C.blue}}>({b.porcentaje}%)</span>
              </button>
            ))}
          </div>

          <div style={{marginBottom:12}}>
            <span style={lbl}>Tipo de operación</span>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setEsNomina(false)} style={{flex:1,padding:"9px",borderRadius:9,border:`2px solid ${!esNomina?C.navy:C.border}`,background:!esNomina?C.navy:C.lgray,color:!esNomina?"#fff":C.gray,fontWeight:"bold",fontSize:13,cursor:"pointer"}}>
                💸 Transferencia normal
              </button>
              <button onClick={()=>setEsNomina(true)} style={{flex:1,padding:"9px",borderRadius:9,border:`2px solid ${esNomina?C.navy:C.border}`,background:esNomina?C.navy:C.lgray,color:esNomina?"#fff":C.gray,fontWeight:"bold",fontSize:13,cursor:"pointer"}}>
                🧾 Nómina
              </button>
            </div>
          </div>
      ...
      <Calc monto={monto} tipo={tipo} cliente={cliente} banco={banco} esNomina={esNomina} />
...
// ══ GRÁFICA ══
function GraficaBarras({ movs }) {
  const semanas = useMemo(()=>{
    const map = {};
    movs.forEach(m=>{
      const d=new Date(m.fecha+"T12:00:00");
      const l=new Date(d); l.setDate(d.getDate()-((d.getDay()+6)%7));
      const k=l.toISOString().slice(0,10);
      if(!map[k]) map[k]={k,ing:0,eg:0};
      if(m.tipo==="ingreso") map[k].ing+=m.montoFinal;
...
// ══ PANTALLAS ══
function Resumen({ cls, ctas, movs, meta, onSetMeta }) {
  ...
  const alertas=[];
  cls.forEach(c=>{
    if(saldoCliente(c,movs)<0) alertas.push(`⚠️ ${c.nombre} tiene saldo negativo`);
    if(saldoCliente(c,movs)>=0&&saldoCliente(c,movs)<500&&movs.some(m=>m.clienteId===c.id)) alertas.push(`🔔 ${c.nombre} tiene saldo bajo`);
  });
    ...
    <div>
      ...
      <div style={{...card,marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          ...
          <button onClick={()=>{setEditMeta(!editMeta);setMetaInput(meta?.toString()||"");}} style={{...btnS(C.lblue,C.navy),padding:"4px 10px",fontSize:11}}>{editMeta?"Cancelar":"Editar"}</button>
        </div>
      ...
      <div>
        ...
        <div style={{...card,background:C.navy,color:"#fff"}}>
          ...
          <div style={{opacity:.6,fontSize:12,marginTop:4}}>
            {(c.bancos||[]).map(b=>`${b.banco}: ${b.porcentaje}%`).join(" · ")}
          </div>
            ...
            <div key={m.id} style={{...card,display:"flex",gap:10,alignItems:"center"}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:C.gray}}>
                  {fmtDate(m.fecha)}{m.banco&&` · ${m.banco}`}{m.esNomina&&" · Nómina"}{m.categoria&&` · ${m.categoria}`}
                </div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontWeight:"bold",color:m.tipo==="egreso"?C.red:C.green}}>{fmt(m.montoFinal)}</div>
                {m.comision>0&&<div style={{fontSize:10,color:C.gold}}>com: {fmt(m.comision)} ({m.pct}%)</div>}
              </div>
            </div>
          );
        })}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div style={{flex:1,cursor:"pointer"}} onClick={()=>setDetalle(c)}>
                ...
                <div style={{fontSize:11,color:C.gray,marginTop:4}}>
                  {(c.bancos||[]).map(b=>(
                    <span key={b.banco} style={{background:C.lblue,color:C.navy,borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:"bold",marginRight:4}}>
                      {b.banco} {b.porcentaje}%
                    </span>
                  ))}
                </div>
        ...
        return (
          <div key={c.id} style={{...card}}>
            ...
            <div style={{display:"flex",gap:6,marginTop:10}}>
              ...
              <button onClick={()=>{if(window.confirm(`¿Eliminar ${c.nombre}?\nSaldo: ${fmt(s)}`))onDel(c.id);}} style={{...btnS(C.lred,C.red),padding:"5px 12px",fontSize:11}}>Eliminar</button>
            </div>
          </div>
        );
      })}
  ...
  return (
    <div>
      ...
      {filtrados.map(m=>{
        ...
        return (
          <div key={m.id} style={{...card,borderLeft:m.revisado?`3px solid ${C.green}`:"none"}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:11,color:C.gray,marginTop:2}}>
                      {cta?.nombre}{cli&&` · ${cli.nombre}`} · {fmtDate(m.fecha)}
                      {m.banco&&` · ${m.banco}`}
                      {m.esNomina&&<span style={{background:C.lgold,color:C.gold,borderRadius:20,padding:"1px 6px",fontSize:10,fontWeight:"bold",marginLeft:4}}>Nómina</span>}
                    </div>
...
function Reportes({ cls, ctas, movs }) {
  ...
  const comPorBanco = BANCOS.map(b=>({
    banco:b,
    com: mr.filter(m=>m.tipo==="ingreso"&&m.banco===b).reduce((a,m)=>a+m.comision,0),
    movs: mr.filter(m=>m.banco===b).length
  })).filter(b=>b.com>0||b.movs>0);

  const now=new Date();
  const maDesde=`${now.getFullYear()}-${String(now.getMonth()===0?12:now.getMonth()).padStart(2,"0")}-01`;
  const maHasta=`${now.getFullYear()}-${String(now.getMonth()===0?12:now.getMonth()).padStart(2,"0")}-${new Date(now.getFullYear(),now.getMonth(),0).getDate().toString().padStart(2,"0")}`;
  ...
  return (
    <div>
      ...
      {comPorBanco.length>0&&<div style={{...card,marginBottom:12}}>
...
function Cierres({ cls, ctas, movs, cierres, onCerrar }) {
  ...
  function cerrar(){
    ...
    onCerrar({id:uid(),fecha:hoy,totalEnCuentas:r.total,dineroClientes:r.dineroC,dineroDisponible:r.disponible,
      ingresosDelDia:mh.filter(m=>m.tipo==="ingreso").reduce((a,m)=>a+m.montoFinal,0),
      egresosDelDia:mh.filter(m=>m.tipo==="egreso").reduce((a,m)=>a+m.montoFinal,0),
      comisionesDelDia:mh.filter(m=>m.tipo==="ingreso").reduce((a,m)=>a+m.comision,0),
      numeroMovimientos:mh.length,
      saldosPorCuenta:ctas.map(c=>({nombre:c.nombre,banco:c.banco,saldo:saldoCuenta(c,movs)})),
      notas,fechaCreacion:new Date().toISOString()});
    setNotas("");
  }
    ...
    <div>
      ...
      {[["📥 Ingresos",fmt(detalle.ingresosDelDia),C.lgreen,C.green],["📤 Egresos",fmt(detalle.egresosDelDia),C.lred,C.red],["💸 Comisiones",fmt(detalle.comisionesDelDia),C.lgold,C.gold],["💼 Dinero clientes",fmt(detalle.dineroClientes),C.lblue,C.blue],["✅ Disponible real",fmt(detalle.dineroDisponible),C.lgreen,C.green],["📋 Movimientos",detalle.numeroMovimientos,C.lgray,C.gray]].map(([l,v,bg,col])=>(
        <div key={l} style={{...card,display:"flex",justifyContent:"space-between"}}>
          <span style={{color:C.gray,fontSize:14}}>{l}</span><span style={{fontWeight:"bold",color:col}}>{v}</span>
        </div>
      ))}
      ...
      <div style={{...card,background:C.lgray,borderLeft:`4px solid ${C.navy}`}}>
        ...
        {[["Total en cuentas",fmt(r.total),C.navy],["Dinero clientes",fmt(r.dineroC),C.blue],["Disponible real",fmt(r.disponible),r.disponible>=0?C.green:C.red]].map(([l,v,col])=>(
          <div key={l} style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:4}}>
            <span style={{color:C.gray}}>{l}</span><span style={{fontWeight:"bold",color:col}}>{v}</span>
          </div>
        ))}
      </div>
      <span style={lbl}>Notas del cierre (opcional)</span>
      <input value={notas} onChange={e=>setNotas(e.target.value)} placeholder="Observaciones del día..." style={{...inp,marginBottom:12}} />
...
const TABS=[
  ...
  {key:"calc",       label:"Calculadora",emoji:"⚡"},
  ...
  return (
    <div style={{fontFamily:"Georgia,serif",minHeight:"100vh",background:"#f7f3ee",paddingBottom:80}}>
      ...
      <div style={{padding:"14px 12px",maxWidth:620,margin:"0 auto"}}>
        {tab==="resumen"     && <Resumen cls={cls} ctas={ctas} movs={movs} meta={meta} onSetMeta={setMeta} />}
        {tab==="calc"        && <CalculadoraRapida clientes={cls} />}
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:C.white,borderTop:`1px solid ${C.border}`,display:"flex",zIndex:20,boxShadow:"0 -2px 8px rgba(0,0,0,0.06)",overflowX:"auto"}}>
        {TABS.map(t=>(
